...
<div id="label-page"><h3>Tampil Data Surat</h3></div>
<div id="content">
	
	<p id="tombol-tambah-container"><a href="index.php?p=surat-input" class="tombol"><button type="button" class="btn btn-dark">Tambah Surat</button></a>
	...
	<table id="tabel-tampil" class="table table-bordered table-sm">
		...
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			if($pencarian != ""){
				$sql = "SELECT * FROM tbsurat WHERE nama LIKE '%$pencarian%'
						OR idsurat LIKE '%$pencarian%'
						OR nosurat LIKE '%$pencarian%'
						OR tentang LIKE '%$pencarian%'
						OR dokumen LIKE '%$pencarian%'
						OR nama_admin LIKE '%$pencarian%'
		...
		<tr>
			...
			<td><?php echo $r_tampil_surat['tentang']; ?></td>
			<td><?php echo $r_tampil_surat['jenis']; ?></td>
			<td><a href="berkassurat/<?php echo $dokumen; ?>"> Lihat Arsip</td>
			<td><?php echo $r_tampil_surat['nama_admin']; ?></td>
			<td>
				<a href="index.php?p=surat-edit&id=<?php echo $r_tampil_surat['id_surat'];?>" class="tombol">Edit</a>|
				<a href="proses/surat-hapus.php?id=<?php echo $r_tampil_surat['id_surat']; ?>" onclick = "return confirm ('Apakah Anda Yakin Akan Menghapus Data Ini?')" class="tombol">Hapus</a></div>
			</td>			
		</tr>		
		<?php $nomor++; } 
		}
